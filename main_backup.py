...
def calculate_KS_stats(args,model,simulator):
    ...
    if args.dataset == 'von_Mises_circle':
        ...
        with torch.no_grad():
            ...
            # import pdb
            # pdb.set_trace()
            diff = density_flow.T - torch.tensor(true_probs).view(n_pts,int(n_pts*multiplier)).to(args.device, torch.float) 
        ...
        x = torch.tensor(x).to(args.device, torch.float)
        logprobs =  []
        # with torch.no_grad():
        model.eval()
        params_ = None
...
if __name__ == '__main__':
    ...
    if args.plot:
        if args.restore_file:
            from utils import load_checkpoint
            model, optimizer, scheduler, args.step = load_checkpoint(args.output_dir,model)
