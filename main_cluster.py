...
def calculate_KS_stats(args,model,simulator,precision=100,tag='test'):
    ...
    if args.dataset == 'von_Mises_circle':
        ...
        with torch.no_grad():
            ...
            # pdb.set_trace()
            probs_flow = probs_flow.view(int(n_pts*multiplier),n_pts) 
            ...
            diff = density_flow - torch.tensor(true_probs).view(int(n_pts*multiplier),n_pts).to(args.device, torch.float) 
...
def train_flow(model, simulator, loss_fn, optimizer, scheduler, args, double_precision=False):
    ...
    for step in range(args.step+1,args.n_gradient_steps):
        ...
        if step%50000 == 0:
            # pdb.set_trace()
            logger.info("Plotting latents for step %s ",step)
