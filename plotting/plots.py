...
def plt_latent_distribution(output_dir,simulator,model,i_epoch=0,n_grid=100,dtype=torch.float,device=torch.device("cpu")):
    ...
    if simulator.latent_dim() == 2:
        ...
        probs = torch.exp(log_prob).view(latent[1].shape[0],latent[0].shape[0]).detach().cpu().numpy() * np.abs(jacobians) # * np.sqrt(2*np.pi*sig2)  <--constant irrelevant for plotting
        np.save(os.path.join(output_dir, 'density_flow_local_gauss.npy'), probs)
        ...
        model_probs = torch.exp(log_prob).detach().cpu().numpy() * np.sqrt(np.abs(jacobians)) * (2*np.pi*0.01)**((simulator.data_dim()-1)/2) * 1
