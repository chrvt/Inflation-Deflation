...
parser.add_argument('--kernel', action='store_true', help='Kernel density estimation.')
# target density
...
if __name__ == '__main__':
    ...
    if args.kernel:
        ...
        samples = simulator._draw_z(100000)
        gkde = stats.gaussian_kde(dataset=samples)
        # probs_model = gkde.evaluate(samples)
        ##----KS stats
        prec = 500 #precision for integrals
        CDF_original = torch.zeros(prec)
        CDF_model = torch.zeros(prec) 
        ##bounds
        data_grid, latent, true_probs, jacobians, multiplier = simulator.generate_grid(2)
        
        a_, b_ = latent[0], latent[1]
        dx = (b_ - a_)/(prec*2)
        dy = 1
        for k in range(1,prec+1):
            b = a_*(prec-k)/(prec) + b_*k/(prec)
            
            z_np = np.linspace(a_,b,1000+1)[1:] #--> thats why prec*2 for dx
            z_torch = torch.tensor(z_np).to(args.device, torch.float)
            
            density_flow = torch.tensor(gkde.evaluate(z_np)).to(args.device, torch.float) #integrand(z_np,model,simulator) 
            CDF_model[k-1] = torch.trapz(density_flow, z_torch)
        ...
        CDF_diff = torch.abs(CDF_model-CDF_original)
        KS_test = torch.max(CDF_diff).cpu().detach().numpy() 
        
        np.save(os.path.join(args.output_dir, 'KS_'+tag+'.npy'), KS_test)
        np.save(os.path.join(args.output_dir, 'density_flow.npy'), density_flow.detach().cpu().numpy()) 
        logger.info("KS statistics: %s", KS_test)  
        logger.info("Finished KDE...have an amazing day!")
        quit()
