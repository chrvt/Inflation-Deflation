...
parser.add_argument('--digit', type=int, default=1, help='What MNIST digit to train?')
...
# target density
parser.add_argument('--dataset', type=str, default="MNIST", help='Which potential function to approximate.')

# model parameters
parser.add_argument('--data_dim', type=int, default=784, help='Dimension of the data.')
parser.add_argument('--hidden_dim', type=int, default=1568, help='Dimensions of hidden layers.')
parser.add_argument('--n_hidden', type=int, default=1, help='Number of hidden layers.')
# training parameters
parser.add_argument('--epochs', type=int, default=10, help='Number iterations through entire train set.')
...
def train_flow(model, simulator, loss_fn, optimizer, scheduler, args, double_precision=False):
    ...
    training_ = torch.load(r'D:\PROJECTS\MNIST_data\MNIST\processed\training.pt')
    data, labels = training_[0].to(args.device,dtype).flatten(1,-1), training_[1]  #[28,28] --> 784
    
    label_idx = ((labels == args.digit).nonzero()).flatten()  #get label
    label_data = data[label_idx,:]
    ##validation split
    total_size = len(label_idx)
   
    val_size = int(np.round(total_size/10))
    train_size = int(total_size-val_size)
    # pdb.set_trace()
    train_set, val_set = torch.utils.data.random_split(label_data,[train_size, val_size])
    
    train_loader = torch.utils.data.DataLoader(train_set,batch_size=args.batch_size, shuffle=True)
    val_loader = torch.utils.data.DataLoader(val_set,batch_size=args.batch_size, shuffle=True)
        
    with tqdm(total=args.epochs, desc='Start step {}; Training for {} steps'.format(args.step,args.epochs)) as pbar:
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, batch in enumerate(train_loader):                 
                model.train()
                if args.sig2 >0:
                    x_tilde = batch + (np.sqrt(args.sig2)*torch.randn(args.batch_size,784))
                else:
                    x_tilde = batch
                ...
                if args.step == 1:
                    plot_MNIST(args.output_dir,simulator,model,i_epoch=step,n_grid=100,dtype=dtype,device=args.device)
...
if __name__ == '__main__':
    ...
    args.output_dir = os.path.join(args.output_dir, args.dataset, param_string) 
    ...
    simulator = None #load_simulator(args)
